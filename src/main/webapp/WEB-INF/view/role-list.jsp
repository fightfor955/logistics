...
<span class="l" id="toolbar">
	...
	<a href="javascript:;" onclick="role_add('添加角色','role/edit.do','800','500')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加角色</a></div>
  	<table id="roleTable" class="table table-border table-bordered table-bg"></table>
</div>
<!--_footer 作为公共模版分离出去-->
...
<script type="text/javascript">
	...
	function operationFormatter(value,row,index){
		...
		var html = "<span onclick='role_edit("+value+")'style='cursor: pointer;' class='glyphicon glyphicon-pencil'></span>&nbsp;&nbsp;&nbsp;";
		
		html += "<span onclick='role_del(this,"+value+")' style='cursor: pointer;color:red' class='glyphicon glyphicon-trash'></span>"
...
function role_del(obj,roleId){
	...
	/* function role_del(userId){
		//layer的弹框使用
		layer.confirm('确认要删除吗？',function(index){
			//如果是点击的确认就会执行函数ajax请求
			$.post("role/delete.do",{"userId":userId},function(data){
				//弹出一个提示消息
...
function menuFormatter(value,row,index){
	if("menu"==value){
		return "<span style='color:red'>菜单角色</span>";
	}
	return "普通角色";
}	
/*角色-编辑*/
